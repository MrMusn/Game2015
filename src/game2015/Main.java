...
public class Main extends Application {
	...
	private static final String NAME = "PsykoDennis";
	...
	private static final String IP_RANGE = "10.10.0.0/16";

	/** If null, scan ips automatically using range {@link Main#IP_RANGE} */
	private final static String[] ipArr = { "10.10.139.229" }; //"10.10.140.228", 
	...
	/**
	 * Initializes connection to all other known clients (known == {@link Main#ips} ) by sending the first name message
	 */
	private void initConnect() {
		new Thread(() -> {
			for (String ip : ips) {
				final String nameStr = "name " + me.getName() + " " + me.getXpos() + " " + me.getYpos();
				writeMsg(nameStr, ip);
			}
		}).start();
	}
	 ...
	 * This is a static method to allow access from the Player class through {@link Main#broadcastPoints(Player, int)}.
	 * 
	 * @param msg
	...
	private void initListen() throws IOException {
		...
		new Thread(() -> {	
			...
			while (true) { //(start + (30 * 1000)) > System.currentTimeMillis()) { //30 seconds to connect
				Socket sock = null;
				try {
		...
		new Thread(() -> {
			...
			while (start + (180 * 1000) > System.currentTimeMillis()) { //180 seconds to connect
				try {
					...
					{
						sock.getOutputStream().write("Who is it?".getBytes("UTF-8"));
				...
				try {this.scanSrvSock.close();} catch (Exception e) {e.printStackTrace();}
			}
		}).start();
	}
	...
	private void regPlayer(final String[] reqLineArr, final InetAddress ip) {
		final Player p = new Player(reqLineArr[1], Integer.parseInt(reqLineArr[2]), Integer.parseInt(reqLineArr[3]), "up", ip);
		synchronized (this) {
			players.add(p);
		}
	...
	private void regPlayerMove(final String[] reqLineArr, final Player player) {
		...
		if (player == null)
			throw new RuntimeException("Unknown player address for position change");
	...
	private void regPlayerPoints(final String[] reqLineArr) {
		...
		if (player == null)
			throw new RuntimeException("Unknown name received for point change.");

		final int pointsChange = Integer.parseInt(reqLineArr[2]) - player.getPoints();
	...
	public void start(Stage primaryStage) {
		try {
			...
			me = new Player(NAME,X_POS,Y_POS,"up");
//			randomizePos(me);
			synchronized (this) {
				players.add(me);
			}
			fields[this.me.getXpos()][this.me.getYpos()].setGraphic(new ImageView(hero_up));
	...
	public void stop() throws IOException {
		if (this.srvSock != null) this.srvSock.close();
		if (this.scanSrvSock != null && !this.scanSrvSock.isClosed()) this.scanSrvSock.close();
	}
	...
	private void randomizePos(Player player) {
		...
		} while (board[player.getXpos()].charAt(player.getYpos()) != 'w');
	}
	...
	public void broadcastMove() {
			for (final String ip : ips)
				writeMsg("move " + me.getXpos() + " " + me.getYpos(), ip);		
	}
